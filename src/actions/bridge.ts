...
export class BridgeAction {
  private readonly L1_BRIDGE_ADDRESS = '0xF05F0e4362859c3331Cb9395CBC201E3Fa6757Ea' as const;
  private readonly L2_BRIDGE_ADDRESS = '0x4000698e3De52120DE28181BaACda82B21568416' as const;
  private readonly LEGACY_ERC20_ETH = '0xDeadDeAddeAddEAddeadDEaDDEAdDeaDDeAD0000' as const;
    ...
    try {
      ...
      // Handle opBNB to BSC bridging
        ...
        if (selfBridge && nativeTokenBridge) {
          ...
          const args = [this.LEGACY_ERC20_ETH, params.toAddress!, amount, 1, '0x'] as const;
          const value = amount + delegationFee;
...
export const bridgeAction = {
  ...
  handler: async (
    ...
    try {
      ...
      const errorMsg = error instanceof Error ? error.message : String(error);
      elizaLogger.error('Error getting wallet info:', errorMsg);
      callback?.({
        text: `Unable to access wallet: ${errorMsg}`,
    ...
    const bridgePrompt = composePromptFromState({
      state: currentState,
    ...
    try {
      ...
      const errorMsg = error instanceof Error ? error.message : String(error);
      elizaLogger.error('Error during token bridge:', errorMsg);
      ...
      } else if (errorMsg.includes('token address on opBNB is required')) {
        errorMessage = `When bridging ERC20 tokens from BSC to opBNB, you must specify the token address on opBNB.`;
      } else if (errorMsg.includes('Unsupported bridge direction')) {
