  ...
  async transfer(params: TransferParams): Promise<TransferResponse> {
    ...
    if (!params.token || params.token == 'null' || params.token === nativeToken) {
      ...
      if (params.token === 'BNB' || params.token === 'bnb') {
        ...
        try {
          elizaLogger.debug(
            `Attempting to resolve token symbol: ${params.token} on chain ${params.chain}`
          );
          // Configure the LI.FI SDK for token lookup
          this.walletProvider.configureLiFiSdk(params.chain);

          tokenAddress = await this.walletProvider.getTokenAddress(params.chain, params.token);
          ...
          throw new Error(
            `Could not find token ${params.token} on chain ${params.chain}. Please check the token symbol or use the contract address.`
          );
        }
      } else {
...
export const transferAction = {
  ...
  handler: async (
    ...
    try {
      ...
      if (errorMsg.includes('LI.FI SDK')) {
        elizaLogger.error('LI.FI SDK error detected');

        if (
          errorMsg.includes('Request failed with status code 404') &&
          errorMsg.includes('Could not find token')
        ) {
          // Extract the token that couldn't be found from the error message
          const tokenMatch = errorMsg.match(/Could not find token (.*?) on chain/);
        ...
        } else if (errorMsg.includes('400 Bad Request') && errorMsg.includes('chain must be')) {
