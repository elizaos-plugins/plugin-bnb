...
// Content interface for faucet action
interface FaucetContent extends Content {
  token?: string;
  toAddress?: string;
}

// Validation function for faucet content
function isFaucetContent(_runtime: IAgentRuntime, content: any): content is FaucetContent {
  elizaLogger.debug('Faucet content for validation', content);
  // Both fields are optional, so we just check they're strings if present
  return (
    content &&
    (content.token === undefined || typeof content.token === 'string') &&
    (content.toAddress === undefined || typeof content.toAddress === 'string')
  );
}
...
export class FaucetAction {
  ...
  async faucet(params: FaucetParams): Promise<FaucetResponse> {
    ...
    try {
      ...
      elizaLogger.error(`Faucet error: ${errorMessage}`, error);
      throw error;
    }
  }
  ...
  async validateAndNormalizeParams(params: FaucetParams): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        const errorMessage = error instanceof Error ? error.message : String(error);
        elizaLogger.error(`Error formatting address: ${errorMessage}`);
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      elizaLogger.error(`Error in validateAndNormalizeParams: ${errorMessage}`);
      throw error;
    }
...
export const faucetAction = {
  ...
  handler: async (
    ...
    // Compose faucet prompt
    const faucetPrompt = composePromptFromState({
      state: currentState,
    ...
    // Validate content
    if (!isFaucetContent(runtime, content)) {
      elizaLogger.error('Invalid content for faucet action.');
      callback?.({
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      elizaLogger.error('Error during faucet:', errorMessage);

      // Provide more user-friendly error messages
      let userMessage = `Get test tokens failed: ${errorMessage}`;
