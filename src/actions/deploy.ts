...
// Content interface for deploy action
interface DeployContent extends Content {
  chain: string;
  contractType: string;
  name: string;
  symbol?: string;
...
// Validation function for deploy content
function isDeployContent(_runtime: IAgentRuntime, content: any): content is DeployContent {
  elizaLogger.debug('Deploy content for validation', content);
  return (
    content &&
    typeof content.chain === 'string' &&
    typeof content.contractType === 'string' &&
  ...
  async compileSolidity(contractName: string, source: string) {
    ...
    try {
      ...
      if (output.errors) {
        ...
        const hasError = errors.some((error: any) => error.type === 'Error');
      ...
      throw new Error(
        `Failed to compile contract: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
...
export const deployAction = {
  ...
  handler: async (
    ...
    try {
      ...
      callback?.({
        text: `Unable to access wallet: ${error instanceof Error ? error.message : String(error)}`,
        content: {
    ...
    // Compose prompt
    elizaLogger.debug('Composing contract template prompt...');
    ...
    // Validate content
    if (!isDeployContent(runtime, content)) {
      elizaLogger.error('Invalid content for deploy action.');
      callback?.({
        text: 'Unable to process deploy request. Invalid content provided.',
    ...
    try {
      ...
      } else if (error instanceof Error && error.message.includes('compilation failed')) {
