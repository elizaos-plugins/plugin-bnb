...
// Content interface for getBalanceTestnet action
interface GetBalanceTestnetContent extends Content {
  chain: string;
  address?: string;
  token?: string;
}

// Validation function for getBalanceTestnet content
function isGetBalanceTestnetContent(
  _runtime: IAgentRuntime,
  content: any
): content is GetBalanceTestnetContent {
  elizaLogger.debug('GetBalanceTestnet content for validation', content);
...
export class GetBalanceTestnetAction {
  ...
  private async checkBalanceViaBscScan(address: string): Promise<string | null> {
    try {
      ...
      elizaLogger.error(
        `Error checking BSCScan balance: ${error instanceof Error ? error.message : String(error)}`
      );
      return null;
    }
  }
  ...
  async getBalance(params: GetBalanceParams): Promise<GetBalanceResponse> {
    ...
    if (!queryNativeToken) {
      ...
      try {
        ...
        elizaLogger.error(
          `Error getting ERC20 balance: ${error instanceof Error ? error.message : String(error)}`,
          error instanceof Error ? error.stack : undefined
        );
        throw error;
      }
      ...
      try {
        ...
        const chainId = await publicClient.getChainId().catch((e) => {
          elizaLogger.error(
            `Failed to get chain ID: ${e instanceof Error ? e.message : String(e)}`
          );
          return null;
        });
        ...
        elizaLogger.error(
          `Error getting native balance: ${error instanceof Error ? error.message : String(error)}`,
          error instanceof Error ? error.stack : undefined
        );
  ...
  async getERC20TokenBalance(
    ...
    try {
      ...
      try {
        ...
        elizaLogger.error(
          `Contract call to balanceOf failed: ${e instanceof Error ? e.message : String(e)}`
        );
      ...
      try {
        ...
        elizaLogger.error(
          `Contract call to decimals failed: ${e instanceof Error ? e.message : String(e)}`
        );
...
export const getBalanceTestnetAction = {
  ...
  handler: async (
    ...
    // Validate content
    if (!isGetBalanceTestnetContent(runtime, content)) {
      elizaLogger.error('Invalid content for getBalanceTestnet action.');
      callback?.({
        text: 'Unable to process testnet balance request. Invalid content provided.',
    ...
    try {
      ...
      try {
        ...
        elizaLogger.error(
          'Error during get testnet balance:',
          error instanceof Error ? error.message : String(error),
          error instanceof Error ? error.stack : undefined
        );

        // Provide more user-friendly error messages based on error type
        let userMessage = `Error checking testnet balance on BSC Testnet: ${error instanceof Error ? error.message : String(error)}`;
        ...
        } else if (error instanceof Error && error.message.includes('No URL was provided')) {
          userMessage =
            'Network connection issue. Please check your BSC_TESTNET_PROVIDER_URL configuration.';
        } else if (error instanceof Error && error.message.includes('Invalid address')) {
          userMessage = 'The address provided is invalid. Please provide a valid wallet address.';
        } else if (
          error instanceof Error &&
          error.message.includes('not supported on BSC testnet')
        ) {
          userMessage = error.message;
        } else if (error instanceof Error && error.message.includes('Contract 0x')) {
          userMessage =
      ...
      callback?.({
        text: `A critical error occurred while checking testnet balance: ${error instanceof Error ? error.message : String(error)}`,
        content: {
          error: error instanceof Error ? error.message : String(error),
          chain: 'bscTestnet',
        },
