...
// Content interface for stake action
interface StakeContent extends Content {
  chain: string;
  action: string;
  amount?: string;
}

// Validation function for stake content
function isStakeContent(_runtime: IAgentRuntime, content: any): content is StakeContent {
  elizaLogger.debug('Stake content for validation', content);
  return content && typeof content.chain === 'string' && typeof content.action === 'string';
}
...
export class StakeAction {
  ...
  async doDeposit(amount: string): Promise<string> {
    ...
    try {
      ...
      elizaLogger.error(`Error during deposit operation:`, errorMessage);
  ...
  async doWithdraw(amount?: string): Promise<string> {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      elizaLogger.error(`Error during withdraw operation:`, errorMessage);
  ...
  async doClaim(): Promise<string> {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      elizaLogger.error(`Error during claim operation:`, errorMessage);
...
export const stakeAction = {
  ...
  handler: async (
    ...
    try {
      ...
      elizaLogger.error('Error during stake:', baseErrorMessage);
